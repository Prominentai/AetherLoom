...
**当前版本：0.2.1** · Built with Python / PyQt5

## 主要功能

- **RunningHub 应用**：通过应用网址生成输入卡片，支持标准模型与 LLM、批量运行、任务队列、进度查询和结果下载。
- **NovelAI 绘图**：独立图像创作页，支持文生图、图生图、遮罩重绘、多角色、参考图、图像工具与本地历史；在页面连接设置中填写 NovelAI API Token。
...
- **提示词工具**：自动补全、翻译、扩写、文本文件读取、随机词与通配符，以及可导入的提示词样式。
- **素材与模型库**：本地文件浏览、媒体预览、图像对比；检索 RH 模型并管理本地收藏。
- **图像与解码**：遮罩绘制与处理、色彩与混合、图像分块合并，以及 GRC / SSTool 本地解码。
